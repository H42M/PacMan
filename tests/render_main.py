...
def reset_ghost_pos() -> None:
    ghost.x = 1
    ghost.y = RenderLoader.screen_size[1] // 2
...
if __name__ == "__main__":
    ...
    ctn_h = Container(screen, 'HORIZONTAL',
                      pos=(0, 0),
                      size=(RenderLoader.screen_size[0], 0),
                      gap=20)
    ctn_h.add_content([
    ...
    # MAZE CONTAINER
    maze = RenderMaze(screen, (RenderLoader.screen_size))
    ctn_maze = Container(screen, 'HORIZONTAL', size=(RenderLoader.screen_size),
                         gap=0)
    ...
    # TOTAL CONTAINER
    ctn_v = Container(screen, 'VERTICAL', (0, 0), RenderLoader.screen_size, 0)
    ctn_v.add_content([
        {ctn_h: '5%'},
        {ctn_maze: '85%'},
        {info_container: '5%'}
    ...
    pacman = Entity(screen, (100, 100), RenderLoader.cell_size)
    pacman.set_skin(RenderLoader.get_asset('pacman'))
