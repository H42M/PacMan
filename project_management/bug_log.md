...
| Active ghosts could overlap. | Medium | Fixed | Added active ghost position reservation during ghost movement; dead ghosts do not block cells. | Fixed late in final cleanup. |
...
| Default config did not match final compliance expectations. | Low | Fixed | Updated default lives/seeds in the default config path during final compliance cleanup. | Keeps launch defaults aligned with expected gameplay. |
...
| Non-functional Settings screen was player-facing. | Low | Fixed | Removed only the main-menu settings entry/callback; settings state registration remains intact. | Avoids exposing fake options. |
...
| Ghost AI is not arcade-perfect. | The subject requires autonomous chase/flee behavior, not exact original Pac-Man AI. The current implementation is path-aware and distinct enough for project scope. |
| Some final packaging validation is still pending. | Packaging is a dedicated finalization task and will be validated after the build script/spec is added. |
